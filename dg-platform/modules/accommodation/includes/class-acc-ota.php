...
class DG_Acc_Ota {
    ...
    /**
     * Recompute booking-derived blocked ranges for AJAX callers.
     * Does NOT overwrite dg_blocked_dates — that field is reserved for manual operator blocks.
     * Live availability already merges bookings + manual via DG_Acc_Frontend::get_blocked_dates().
     *
     * @return string[] ranges like "YYYY-MM-DD to YYYY-MM-DD"
     */
    public static function rebuild_blocked_dates($accommodation_id) {
        ...
        update_post_meta($accommodation_id, 'dg_ota_blocked_dates', implode("\n", $blocked_ranges));
        update_post_meta($accommodation_id, 'dg_ota_blocked_rebuilt_at', current_time('mysql'));
